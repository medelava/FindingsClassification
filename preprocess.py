...
def resize_and_center_fundus(original_img_path=None, preprocessed_img_path=None, diameter=299, verbosity=1):
    """
    ...
    if preprocessed_img_path is None:
        raise ValueError("preprocessed_images path not specified!")

    #preprocessed_img_path = os.path.abspath(preprocessed_img_path)
   
    # Get the paths to all images.
    image_paths = [os.path.join(original_img_path, fn) for fn in os.listdir(original_img_path)]
    ...
    for i, image_path in enumerate(image_paths):
        ...
        try:
            ...
            if processed is None:
                ...
                split_name = os.path.splitext(os.path.basename(image_filename))
                if split_name[1]=='.JPG':
                    image_filename=split_name[0]+'.jpg'
                
                output_path = os.path.join(preprocessed_img_path, image_filename)
...
if __name__=='__main__':  
    ...
    parser.add_argument("--preprocess_info", type=str, default="./experiments/parameters/preprocess_info.json")
    ...
    res = resize_and_center_fundus(**preprocess_info)
