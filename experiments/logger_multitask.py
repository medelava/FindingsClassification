    ...
    def __call__(self, X_test, y_true, model, network_weights_name, clasfier, test_steps, **kwargs):
        ...
        if type(X_test)==list:
            ...
            for x,y in X_test.take(test_steps):
                ...
                y_pred_round.append(_np.round(prediction))
                y_true.extend(y)
        
        df = _pd.DataFrame([])
        
        for i in range(y_pred_test.shape[1]):
            f1_macro = f1_score(y_true[:,i], y_pred_round[:,i], average="macro")
            f1_micro = f1_score(y_true[:,i], y_pred_round[:,i], average="micro")
         
            con_mat = confusion_matrix(y_true[:,i], y_pred_round[:,i])
            tp = con_mat[1][1]
            tn = con_mat[0][0]
            fp = con_mat[0][1]
            fn = con_mat[1][0]
            ...
            auc = roc_auc_score(y_true[:,i], y_pred_test[:,i])
            
            df = df.append([[network, clasfier, auc, recall, specificity, acc, f1_macro, f1_micro, network_weights_name, kwargs['optimizer'], kwargs['dropout'], kwargs['learning_rate'], kwargs['epochs']]])
